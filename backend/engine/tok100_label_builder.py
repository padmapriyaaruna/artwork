# -*- coding: utf-8 -*-
"""
TOK100 Label Builder v12  -  exact match to TOK100_B0854559_1.pdf actual output.

All coordinates derived by scientific extraction from the actual approved PDF.

ACTUAL PDF measurements (page 2004.1x1417.3pt, first back panel):
  Panel oy=373.25, ox=581.85, OUTER_W=150.3, OUTER_H=305.5
  INNER_X=11.5, INNER_W=127.2  (ix0=593.35, ix1=720.55)
  
  Panel-relative Y coordinates:
    YEARS val baseline  : 148.0
    IT val baseline     : 159.1
    SizeChip row 1 bl   : 173.3
    SizeChip row 2 bl   : 185.3
    Dept codes baseline : 200.6
    Barcode digit str   : 227.4
    Style code          : 236.7
    Cref                : 246.1
    Price EUR+29 bl     : 284.5
    Cents ,95 bl        : 276.7  (raised 7.8pt)
    
  TABLE horizontal lines:
    TABLE_TOP : ~138 (YEARS bl 148 - ~10)
    TABLE_BOT : ~168 (IT bl 159 + ~9)
    Lines span ix0..ix1 minus 0 inset (same width as KIDS separators)
    
  SIZE CHIP:
    Pitch (CHIP_W + CHIP_GAP) = 19.06pt
    Row 1: 4-5@x=638.30, 5-6@x=657.36, 6-7@x=675.97  y=546.55
    Row 2: 7-8@x=637.08, 8-9@x=655.44, 9-10@x=674.04 y=558.57
    chip_x0 (actual page) = 638.30 → rel = 638.30 - 593.35 = 44.95 from ix0
    chip_x0 rel from ix0 = 44.95  
    CHIP_W ≈ 13pt (estimated from visual; pitch 19 - GAP 6 = 13)
    
  BARCODE:
    Dept codes above bars AT y_rel=200.6 (same as zone B2 start = 188+12.6)
    Type3 font barcode in actual — we use PNG; display width should match mc_w
    Bars: from y_rel=207 to y_rel=223 (approx, 16pt bar height in actual)
    Digit string below at y_rel=227.4
    
  PRICE:
    EUR at x=631.51 (page abs) -> rel_x = 631.51 - 593.35 - 11.5(INNER_X) = 26.66 from ix0
    29  at x=650.14 -> rel_x = 45.29 from ix0 -> gap from EUR right ≈ 2pt
    Price group starts at ix0 + 26.66 = 620.01 page abs
    Price centred in inner width: icx = ix0 + INNER_W/2 = 593.35 + 63.6 = 656.95
    Total price width: (673.69 + tw(,95 at 15.1)) - 631.51
      tw(,95 at 15.1) ≈ 12pt -> right edge = 685.69
      total_w = 685.69 - 631.51 = 54.18pt
      centre = 631.51 + 54.18/2 = 658.6 ≈ 656.95 ✓ (centred in inner area)

v12 specific fixes:
  1. TABLE borders: use SEP_INSET=0 so they match KIDS separator length exactly
     (KIDS separator and table borders span ix0 to ix1 with NO inset)
  2. SIZE CHIP: CHIP_W=13pt, CHIP_GAP=6pt (pitch=19), centred from ix0+44.95
     Active chip padding: 1pt top, 1pt bottom (very compact, matches actual)
  3. MAGENTA BORDER: restore bottom corners by drawing full bottom+side strokes
  4. BARCODE: wider display across full mc_w, BAR_H=16pt matching actual
  5. PRICE: EUR uses same font (FR/FB) with gap=2pt before digits
             currency is the actual symbol char (auto via _fix_currency)
             NO text fallback like "AED" in the display — always a symbol
  6. GREEN SEPARATOR: dashes are [3 3] matching actual; Y position correct
"""
import io, os, re
import fitz  # PyMuPDF
